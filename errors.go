...
func WrapError(cause error, msg ...string) *Error {
	newMsg := ""
	if len(msg) > 2 {
		newMsg = fmt.Sprintf(msg[0], msg[1:])
	} else if len(msg) == 1 {
		newMsg = msg[0]
	}
	...
	if err, ok := cause.(*Error); ok {
		if newMsg != "" {
			err.cause = errors.Wrap(err.cause, newMsg)
		} else {
			err.cause = errors.WithStack(err)
		}
		return err
	}
	...
	var base error
	if newMsg != "" {
		base = errors.Wrap(cause, newMsg)
	} else {
		base = errors.WithStack(cause)
	}
	return newError(base)
}
